...
class index extends Component {
  ...
  render(){
    let autoCompleteList = this.state.autoCompleteResults.map((response) => {
      return <ul className="users" key={response.id}>
        <li>
          <img alt={response.name} className='gravatar' src={response.gravatar_url} />
          <a href={response.url}>{response.name}</a>
            | <a data-confirm="You sure?" rel="nofollow" data-method="delete" href={response.url} data-remote="true" className={"user-" + response.id}>delete</a>
        </li>
      </ul>
    });
